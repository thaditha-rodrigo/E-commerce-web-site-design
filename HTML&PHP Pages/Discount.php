...
?>
	...
	<?php
	 ...
	 $sql = "SELECT * , `price` - (`price` * `discount`) AS disc_price FROM `product` WHERE `discount` > '0';";
			...
			<div class="column">
				<div class="card" onClick="location.href='item.php?id=<?php echo $row["pro_id"];?>'">
				   <form action="#" method="post">
					  <img src="<?php echo $row["img_path"];?>" style="width:100%;height: 220px;">
					   ...
					   ?>
						 <div class="add-to-wishlist">
							 <a href="addToWishlistManagerDiscount.php?id=<?php echo $row["pro_id"];?>"><img src="images/favorite.png" style="width:35px;height: 35px;"></a>
					  	 </div>
					   <?php
						}
					   ?>
					  ...
					  <p>Rs.<s style="color: red;font-color:black;"><?php echo $row["price"];?></s></p>
					  <p class="price">Rs.<?php echo round($row["disc_price"],2);?></p>
				  	<?php
						if( (isset($_SESSION["username"])) || (isset($_SESSION["admin_id"])) )
						{
					?>
							<p><button name="cart"><a href="addToCartManagerDiscount.php?id=<?php echo $row["pro_id"];?>">Add to Cart</a></button></p>
					<?php
						}
					?>
				  </form>
